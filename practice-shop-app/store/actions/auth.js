...
export const signup = (email, password) => {
  return async (dispatch) => {
    const response = await fetch(
      'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyA24wfOjRqyNM4O_pebBtNSSDLm41T0GsQ',
      {
        method: 'POST',
        headers: {
...
export const login = (email, password) => {
  return async (dispatch) => {
    const response = await fetch(
      'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyA24wfOjRqyNM4O_pebBtNSSDLm41T0GsQ',
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
